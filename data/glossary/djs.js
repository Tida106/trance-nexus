...
For DJs, carrying tracks the audience desperately wants to ID — but cannot find anywhere on streaming services — is itself a form of distinction. The DJ who routinely plays exclusive promos from in-demand producers signals industry access that ordinary subscription music libraries cannot replicate, and the resulting "what was that?" reaction from the floor is part of the value of the set. Conversely, for producers, a buzz-generating ID rotation across multiple top-tier DJs is one of the most reliable signals that a track is going to land at retail.`,
      ja: `「Track ID？」はダンスミュージックのファンが、DJセット、ライブストリーム、公開ミックスで馴染みのないレコードを聴いたあと——会場でもオンラインでも——最も頻繁に尋ねる質問である。答え——トラックのタイトル、アーティスト、理想的にはレーベルやリリース状況——がトラックIDだ。フレーズはコミュニティで独立したスラングになった：「0:43 — track id?」とだけ書かれたYouTubeコメントは、読む者全員に即座に理解される。サブフォーラムやDiscordサーバーがそれだけのために存在する。密接に関連するグロッサリー項目「ID（未公開トラック）」との区別に注意：そちらの項目はまだ名前を付けられないトラックのためのプレースホルダーを扱い、本項目はそれらに名前を付けようとする行為を扱う。

コミュニティはトラック識別をクラウドソースするための精巧なインフラを構築してきた。1001Tracklistsは中心的なグローバル・データベースだ：ユーザーが主要DJセット、フェスティバル、ラジオ放送、ライブストリームのトラックリストを書き起こし、未知の瞬間を「ID」としてマークし、付随するフォーラムで誰かがレコードを特定するまで議論する。MixesDBはウィキ風の構造で類似の役割を果たし、古いセットに特に強い。TrackSniffやその他の新しい音声認識ツールは、短いクリップからの識別を自動化することを試みている。Shazam——消費者向けオーディオ・フィンガープリント・アプリ——は完成した商業リリースに対してはうまく機能し、リスナーが最初に試すことが多いが、構造的な限界がある：まだ未リリースのトラック（「ID」プレースホルダー）は定義上Shazamのフィンガープリント・データベースに存在しないため、任意のセットで最も求められているIDこそ、Shazamが解決できないものだ。

データベースを越えて、リアルタイムの識別は主要な放送をフォローするDiscordサーバー、サブレディット、Facebookグループで起こる。ASOT、Group Therapy、Future Sound of Egyptのエピソード中、リスナーはオンエアから数秒以内に未知トラックの短いクリップを投稿し、より広いライブラリ、レーベル・コネクション、あるいは単により優れた認識記憶を持つコミュニティ・メンバーが、放送をまたいでIDを呼び出す。一部のIDは数分以内に解決される；他は数ヶ月から数年にわたって持続し、未解決のまま民俗学的とも言える地位を獲得し、新しいクリップが浮上するたびにファンが戻ってくるトラックとなる。

DJにとって、観客が必死にIDしたい——しかしどこのストリーミング・サービスでも見つけられない——トラックを持ち運ぶこと自体が、ある種の区別の形である。需要の高いプロデューサーから独占プロモを日常的にプレイするDJは、通常のサブスクリプション音楽ライブラリでは複製できない業界アクセスを示しており、フロアからの「あれは何だった？」という反応がセットの価値の一部だ。逆にプロデューサーにとって、複数のトップ・ティアDJを横断するバズを生むIDローテーションは、トラックがリテールで着地することを示す最も信頼できるシグナルのひとつである。`,
    },
...
Official tracklists exist as well, and the publication policy varies by show. A State of Trance, Group Therapy Radio, Future Sound of Egypt, and Anjunadeep's various podcasts all publish official cuesheets on their show pages, partly because the residencies are licensed broadcasts and partly because the audience expects them — readers can cross-check these against the related <a href="/glossary/asot">A State of Trance</a> entry and the broader <a href="/glossary/set">set</a> and <a href="/glossary/track-id">track-ID</a> infrastructure. Festival mainstage sets, by contrast, are almost always left to the community to transcribe, with the DJ sometimes confirming IDs after the fact on social media. The result is a hybrid ecosystem: a backbone of authoritative show-tracklists from established radio properties, plus a much larger crowdsourced layer that covers everything else.`,
      ja: `トラックリストは、DJセット、ミックス・コンピレーション、またはラジオ番組でプレイされたトラックの順序付きリストである——通常は番号付けされ、しばしば各トラックがミックスに入った時点のタイムスタンプ付き、時には「w/」（同時にプレイ）、「into」（次のトラックへミックス）、「mashup」（別のレコードと組み合わせ）といったトランジション・ノートが注釈される。トラックリストはセットのテキスト文書であり、そのオーディオ録音とは別物だ——選曲と順序を捕捉するが、実際のブレンドは捕捉しない。リスナーにとってトラックリストは、楽しんだが匿名のラジオ・ショウやフェス映像を個別に追跡できるレコードのリストに変えるものだ。DJにとって、トラックリストをリリースするか保留するかという選択自体が、セットがその後どのように消費されるかを形作る判断となる。

トラックリストのための中心的なグローバル・インフラはベルリン在住のドイツ人ソフトウェア開発者Jahn Baers（後にEvan SacksとJacob Merlinがビジネス側に合流）が2011年に立ち上げた1001Tracklistsだ。プラットフォームはクラウドソース——登録ユーザーなら誰でもセットのトラックリストを投稿し、未知のトラックにIDを提案し、正しい識別に投票できる——で、現在ではラジオ・ショウ、フェスのメインステージ、クラブ・レジデンシー、ポッドキャスト、ライブ配信を網羅する260,000本以上のトラックリストをホストしている。各トラックリストのページには関連するYouTubeまたはSoundCloudのオーディオが埋め込まれ、タイムスタンプはクリック可能で、データベース全体でのトラックごとの統計が集計され、同社が2018年からAmsterdam Dance Event期間中に主催するTop 101 Producersの年間ランキングに供給される。

フォーマットの慣習はシーン全体で驚くほど一貫している。各行は通常、ポジション番号、タイムスタンプ（セット開始からのHH:MM:SSまたはMM:SS）、アーティスト（または共演アーティスト連鎖）、トラック・タイトル、角括弧内のレーベルを担う。未識別のトラックはコミュニティまたはDJが正体を明かすまで<em>ID — ID</em>として現れる。エディット、リミックス、マッシュアップのステータスはタイトル・フィールドにフラグが立てられる。同じセットを異なるステージで改訂したバージョンは個別のトラックリストとして現れるため、研究者はある選曲が例えばASOT放送とその後の同素材のフェス・パフォーマンスの間でどう進化するかを比較できる。

公式トラックリストも存在し、公開ポリシーはショウによって異なる。A State of Trance、Group Therapy Radio、Future Sound of Egypt、Anjunadeepの各種ポッドキャストはすべて公式キューシートをショウ・ページに公開している——レジデンシーがライセンスされた放送であることと、観客がそれを期待していることの両方が理由だ——読者はこれらを関連の<a href="/glossary/asot">A State of Trance</a>項目、より広い<a href="/glossary/set">set</a>と<a href="/glossary/track-id">track-ID</a>のインフラと照合できる。フェスのメインステージ・セットは対照的にほぼ常にコミュニティに書き起こしを任され、DJが後からソーシャルメディアでIDを確認することもある。結果としてハイブリッドなエコシステムが生まれている：確立されたラジオ資産からの権威ある番組トラックリストの背骨と、それ以外すべてを覆うはるかに大きなクラウドソースのレイヤーだ。`,
    },
    related: ['set', 'track-id', 'id', 'asot', 'mix-compilation'],
    artists: ['armin-van-buuren', 'above-and-beyond'],
...
クロージング・スロットは、また最も個人的な選曲が現れる場所でもある。夜の早い時間にDJは可能な限り広い観客にプレイし、新鮮な観客を動かすものに制約される；クロージング・スロットは残ることを選んだ自己選別されたコアにプレイされ、DJはより古い素材、より長尺のプログレッシブ・カット、ピーク・タイムでは死ぬであろうヴォーカル・レコード、そして観客への迎合ではなく個人的な声明として読まれるたぐいの選曲へとさらに手を伸ばすことができる。トランス・ファンのリスニング人生で最も愛おしく記憶される瞬間の多くは、夜のこの部分から来ている。`,
...

  {
    ...
    definition: {
      en: `A warmup set — also called an opening set — is the first DJ slot of a night, played before the peak-time and headline artists. Despite the name, the warmup DJ's job is not "to play smaller tracks" or "to fill time before the real DJ arrives." It is a distinct craft with its own rules, and an experienced warmup DJ is a separately respected specialist within the booking ecosystem rather than an undersized version of a headliner.

The core job is to ease the early crowd into the room, build the floor gradually from lower-energy material, and leave the audience and the next DJ in exactly the right place for the energy to climb further. The widely-cited rule of thumb in the literature — including in Bill Brewster and Frank Broughton's How to DJ Right and across two decades of subsequent DJ writing — is that a good warmup should bring the room to roughly 6 or 7 out of 10 by the end of the slot. Lower than that and the headliner has to spend the first twenty minutes of their set doing warmup work that should already be done; higher than that and there is nowhere left for the headliner to take the room. The cardinal sins of the role are mirror-image failures: a DJ who plays too softly and leaves the floor under-built, and a DJ who plays too aggressively, drops anthems, and steals the moment from the headline act.

Track selection during a warmup tends to lean on deeper, more atmospheric, more progressive material than the rest of the night. In trance contexts, a warmup will typically work with progressive-trance and Anjunadeep-style records at 124–130 BPM, withholding the 138 BPM uplifting catalogue and the named anthems until the main DJ takes over. Tempo, key, and energy all rise gradually across the slot, and the final fifteen to twenty minutes are usually shaped specifically to hand off to the next DJ at a tempo and intensity they have indicated.

Etiquette also matters off the decks. Warmup DJs do not play the headliner's biggest tracks (those are slotted into the headliner's planned set), do not play the obvious anthems of the genre regardless of provenance, and do not push their own peak material in front of an audience that is not ready for it. Headliners frequently thank their warmup DJs by name during their own sets, because they know a properly executed warmup makes their entire performance significantly easier to land — and a botched warmup makes it nearly impossible to recover.`,
      ja: `ウォームアップ・セット——オープニング・セットとも呼ばれる——は、ピークタイムおよびヘッドライン・アーティストの前にプレイされる夜の最初のDJスロットだ。名前にもかかわらず、ウォームアップDJの仕事は「小さなトラックをプレイすること」や「本物のDJが到着する前の時間を埋めること」ではない。独自のルールを持つ別個の工芸であり、経験を積んだウォームアップDJは、縮小版のヘッドライナーではなく、ブッキング・エコシステムの中で別個に尊敬される専門職である。

中核の仕事は、早めの観客を部屋に慣らし、より低エネルギーの素材からフロアを徐々に構築し、観客と次のDJをエネルギーがさらに上昇する正しい場所に置いておくことだ。広く引用される経験則は——Bill BrewsterとFrank Broughtonによる『How to DJ Right』をはじめ、その後20年のDJに関する文章の中で——優れたウォームアップはスロットの終わりまでに部屋を10段階のおよそ6〜7まで持ち上げるべきだというものだ。それ未満ならヘッドライナーは自分のセットの最初の20分を、本来すでに済んでいるべきウォームアップ作業に費やさなければならない；それ以上ならヘッドライナーが部屋を持っていく先がもう残っていない。役割の大罪は鏡像的な失敗である：ソフトすぎてフロアを構築しきれないDJと、攻撃的すぎてアンセムを落とし、ヘッドライン・アクトから瞬間を奪うDJ。

ウォームアップ中の選曲は、夜の他の時間よりもよりディープで、より大気的で、よりプログレッシブな素材に傾く傾向がある。トランスの文脈ではウォームアップは典型的に124〜130 BPMのプログレッシブ・トランスやAnjunadeepスタイルのレコードで作業し、138 BPMのアップリフティング・カタログと名指しのアンセムは、メインDJが引き継ぐまで保留される。テンポ、キー、エネルギーはすべてスロットを横断して徐々に上昇し、最後の15〜20分は通常、次のDJが示したテンポと強度で受け渡しできるよう特別に形作られる。

エチケットはターンテーブルの外でも重要だ。ウォームアップDJはヘッドライナーの最大のトラックをプレイしない（それらはヘッドライナーの計画されたセットに組み込まれている）、出所に関係なくジャンルの明白なアンセムをプレイしない、そして観客がまだ準備できていないところに自分のピーク素材を押し出さない。ヘッドライナーはしばしば自分のセット中にウォームアップDJに名前で感謝する——適切に実行されたウォームアップが自分のパフォーマンス全体を大幅に着地しやすくすることを知っているからだ。逆に台無しになったウォームアップから回復することはほぼ不可能である。`,
    },
