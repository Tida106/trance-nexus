...
export const djs = [
...

  {
    ...
    definition: {
      ...
      ja: `DJセットは、DJが一連のトラックを単一の一貫した流れにミックスする連続パフォーマンスだ。セットは長さと意図において大きく異なる：クラブのオープニング・スロットは60分、ピークタイムのフェスティバル・ヘッドラインは90分から2時間、オープン・トゥ・クローズのマラソン・セット（Markus Schulzのスペシャリティ）は6、8、または10時間に及ぶことがある。セットの芸術はトラック選択だけでなく、物語的アークにある——オープニング・トラックを通じてエネルギーを構築し、観客を情緒的なピークと谷を通じて連れ、アンセムを戦略的に配置し、リアルタイムで部屋を読む。録音されたセットはしばしばポッドキャスト、ミックス・コンピレーション、またはビデオ・アップロード（YouTubeのCercleが顕著な例）としてリリースされ、ライブ・パフォーマンスの永続的な記録を作り出す。`,
    },
